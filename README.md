...
MCP server that gives AI full access to your knowledge graph. Supports **Logseq** (read-write) and **Obsidian** (read-only). Navigate pages, search blocks, analyze link structure, track decisions, manage flashcards, and write content — all through the [Model Context Protocol](https://modelcontextprotocol.io).
...
32 tools across 8 categories. Most work with both backends; some are Logseq-only.
...
| Tool | Backend | Description |
|------|---------|-------------|
| `create_page` | Logseq | New page with properties and initial blocks |
| `upsert_blocks` | Logseq | Batch create with nested children for deep hierarchies |
| `update_block` | Logseq | Replace block content by UUID |
| `delete_block` | Logseq | Remove block and all children |
| `move_block` | Logseq | Reposition before, after, or as child of another block |
| `link_pages` | Logseq | Bidirectional link with optional relationship context |

### Decision

| Tool | Backend | Description |
|------|---------|-------------|
| `decision_check` | Logseq | Surface open, overdue, and resolved decisions with deadline status |
| `decision_create` | Logseq | Create a DECIDE block with `#decision` tag, deadline, options, context |
| `decision_resolve` | Logseq | Mark a decision as DONE with resolution date and outcome |
| `decision_defer` | Logseq | Push deadline with reason, tracks deferral count, warns after 3+ |
| `analysis_health` | Logseq | Audit analysis/strategy pages for graph connectivity (3+ links or has decision) |
...
The Obsidian backend is read-only. It parses YAML frontmatter into properties, builds a block tree from headings, and indexes `[[wikilinks]]` for backlink resolution. Daily notes are detected from a configurable subfolder (default: `daily notes`).
...
- **Heading-based blocks for Obsidian.** Obsidian markdown is sectioned by headings (H1-H6) into a hierarchical block tree. Block UUIDs are deterministic from `sha256(filepath:lineNumber)`.
